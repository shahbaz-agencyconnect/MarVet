...
public class PreliminaryMerVetServiceRequest extends AbstractComponent {
	WebDriver driver;

	public PreliminaryMerVetServiceRequest(WebDriver driver) {
		super(driver);
		this.driver = driver;
	...
	@FindBy(xpath = "//button[text()=' PRELIMINARY ']")
	WebElement preliminaryBtn;

	@FindBy(css = ".swal2-confirm")
	WebElement confirmBtn;

	@FindBy(xpath = "//span[text()='Preliminary']")
	WebElement preliminaryTab;
	@FindBy(xpath = "//a[text()=' Continue Vet ']")
	WebElement continueVet;

	public void preliminaryService() throws IOException, TesseractException, InterruptedException {
		ArrayList<String> vesselEntries = excelRead("Preliminary Request");
		...
		preliminaryBtn.click();
		confirmBtn.click();
		preliminaryTab.click();
		Thread.sleep(2000);
		searchBox.sendKeys(imoNum);
		searchIcon.click();
		Thread.sleep(2000);
		String actualImo = driver.findElement(By.xpath("(//table[contains(@id,'pr_id_')]//td)[2]")).getText();
		Assert.assertEquals(actualImo, imoNum);
		actionBtn.click();
		continueVet.click();
	}
}
